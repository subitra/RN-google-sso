...
- (ClientConfiguration)readConfigurationWithOptions:(NSDictionary *)options error:(NSError **)error {
  ...
  if (!iosClientId || shouldAttemptWebClientIdDetection) {
    ...
    iosClientId = iosClientId ?: firPlist[kClientIdKey] ?: mainIosClientID;
    webClientId = firPlist[@"WEB_CLIENT_ID"];
    BOOL offlineAccessRequested = [options[@"offlineAccess"] boolValue];
    if (offlineAccessRequested && !webClientId) {
      if (error) {
        NSString *message = [NSString stringWithFormat:@"Successfully read %@ in order to determine 'webClientId' for offline access.\n"
                             "However, the expected `WEB_CLIENT_ID` entry was not present in the file. Add the entry manually.", pathName];
  ...
  if (mainIosClientID != nil && iosClientId != nil && ![mainIosClientID isEqualToString:iosClientId]) {
    if (error) {
      NSString *message = [NSString stringWithFormat:@"An ios client ID was specified in Info.plist (%@) and a different one was determined by the `configure()` call (%@).\n"
                           "To fix this, remove `iosClientId` from `configure()`. If you use Firebase, make sure your configuration file is up to date.", mainIosClientID, iosClientId];
      *error = [NSError errorWithDomain:@"RNGoogleSignin" code:1002 userInfo:@{NSLocalizedDescriptionKey: message}];
    }
    return emptyConfig;
  }
