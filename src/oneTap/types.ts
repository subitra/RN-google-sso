...
export type OneTapConfigureParams = ClientIdOrPlistPath & {
  ...
  scopes?: string[];
  /**
   * The web client ID obtained from Google Cloud console. In the Modern module only, pass `autoDetect` to automatically determine the value from Firebase config file.
   */
  webClientId: WebClientId;
  // webClientId is required on the web, so we'll make it required here as well

  /**
   * iOS only. Use `requestAuthorization` on Android. Must be true if you wish to access user APIs on behalf of the user from your own server.
   *
   ...
   * The refresh token allows the server to access Google data on behalf of the user. @default false
   */
...
/**
 * this is the public interface of the Modern module
 * @hidden
 * */
 ...
 *
 * @group Web Modern sign in module
 * */
